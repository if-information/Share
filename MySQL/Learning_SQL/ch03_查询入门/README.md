...
当使用术语"表"的时候, 大多数人联想到的是存储在数据库中的关联行的集合, 但这实际上只是表的类型之一而已. 在对"表"的宽泛的定义下, 存在3种类型的表:
...
## 3.5 where子句
在大多数情况下, 并不需要提取表中的每一行, 而是希望使用某种方式过滤掉不感兴趣的行, 这就是`where`子句的作用.   
...
前面的查询都是仅仅提取数据而未对他们进行任何加工. 不过有时候也会需要数据库服务器在返回结果集之前对数据进行一个提炼, 其中一个方式是使用 `group by` 子句, 它用于根据列值对数据进行分组. 举例来说, 也许你并不需要查看雇员和他们所处部门的列表, 而是想要获取部门和它所拥有雇员数的清单. 在使用 `group by` 子句时, 可能还需要同时使用 `having` 子句, 它能够以与 `where` 子句类似的方式对分组数据进行过滤.
...
在排序时, 可以通过关键字 `asc` 和 `desc` 指定是升序还是降序. 由于默认情况下是升序排序, 因此只需要在想要降序排序时加上 `desc` 关键字即可.
